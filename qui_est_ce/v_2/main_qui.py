...
def deroulement_partie(liste):
    ...
    print("LISTE ATTRIBUT APRES :", liste_attributs_mix)
...
def next_attribute(themes):
    if themes == 'habitats' and 'current_index' in session:
        ...
        if current_index < len(session['attributs_mix']):
            session['current_index'] = current_index
            next_attribute = session['attributs_mix'][current_index]
            ...
            return jsonify(attribute=next_attribute, eliminable_images=eliminable_images)
        else:
            return jsonify(end=True)
    return jsonify(error="Invalid session or theme"), 400
